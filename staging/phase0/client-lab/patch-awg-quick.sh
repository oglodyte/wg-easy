...
grep -Fq 'cmd "${WG_QUICK_USERSPACE_IMPLEMENTATION:-amneziawg-go}" "$INTERFACE"' "$target"
...
sed -i.bak \
  ...
  -e 's#cmd "${WG_QUICK_USERSPACE_IMPLEMENTATION:-amneziawg-go}" "$INTERFACE"#cmd env WG_I_PREFER_BUGGY_USERSPACE_TO_POLISHED_KMOD=1 "${WG_QUICK_USERSPACE_IMPLEMENTATION:-amneziawg-go}" "$INTERFACE"#' \
  "$target"
rm -f -- "$target.bak"
...
grep -Fq 'cmd env WG_I_PREFER_BUGGY_USERSPACE_TO_POLISHED_KMOD=1 "${WG_QUICK_USERSPACE_IMPLEMENTATION:-amneziawg-go}" "$INTERFACE"' "$target"
