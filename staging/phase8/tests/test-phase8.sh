...
grep -Fq 'docker/setup-qemu-action@96fe6ef7f33517b61c61be40b68a1882f3264fb8 # v4' \
  "$WORKFLOW"
...
if grep -Eq 'uses: [^#[:space:]]+@v[0-9]+' "$WORKFLOW"; then
  echo "The immutable-image workflow must pin third-party actions by commit." >&2
  exit 1
fi
